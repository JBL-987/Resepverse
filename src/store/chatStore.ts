    ...
    (set, get) => ({
      ...
      sendMessage: async (content, receiverAddress, isEncrypted = false) => {
        ...
        let threadExists = threads.some(t => t.id === targetThreadId && 
          t.participants.includes(receiverAddress));
        
        if (!threadExists) {
          targetThreadId = get().createThread([currentUser.address, receiverAddress]);
        }
          ...
          threads: state.threads.map(t => 
            t.id === targetThreadId
              ? {
                  ...
                  lastActivity: Date.now(),
                  unreadCount: t.id === state.activeThreadId ? 0 : t.unreadCount + 1
                }
              : t
          )
        }));
            ...
            t.id === targetThreadId
              ? {
                  ...
                  messages: t.messages.map(m => 
                    m.id === newMessage.id
                      ? { ...m, transactionHash: `0x${Math.random().toString(36).substr(2, 64)}` }
                      : m
                  )
                }
              : t
